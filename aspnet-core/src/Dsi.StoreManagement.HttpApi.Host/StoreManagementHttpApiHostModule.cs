...
using Dsi.StoreManagement.MultiTenancy;
...
public class StoreManagementHttpApiHostModule : AbpModule
{
            ...
            Configure<AbpVirtualFileSystemOptions>(options =>
            {
                options.FileSets.ReplaceEmbeddedByPhysical<StoreManagementDomainSharedModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Dsi.StoreManagement.Domain.Shared"));
                options.FileSets.ReplaceEmbeddedByPhysical<StoreManagementDomainModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Dsi.StoreManagement.Domain"));
                options.FileSets.ReplaceEmbeddedByPhysical<StoreManagementApplicationContractsModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Dsi.StoreManagement.Application.Contracts"));
                options.FileSets.ReplaceEmbeddedByPhysical<StoreManagementApplicationModule>(
                    Path.Combine(hostingEnvironment.ContentRootPath,
                        $"..{Path.DirectorySeparatorChar}Dsi.StoreManagement.Application"));
            });
        }
    }
    ...
    private void ConfigureAuthentication(ServiceConfigurationContext context, IConfiguration configuration)
    {
        context.Services.AddAuthentication()
            .AddJwtBearer(options =>
            {
                ...
                options.Audience = "StoreManagement";
    ...
    //         {
    //                 {"StoreManagement", "StoreManagement API"}
    //         },
    //         options =>
    //         {
    //             options.SwaggerDoc("v1", new OpenApiInfo { Title = "StoreManagement API", Version = "v1" });
