...
{
    public class AbpDzMessagingervice : Volo.Abp.DependencyInjection.ITransientDependency
    {
        ...
        public AbpDzMessagingervice(
            IHubContext<AbpDzMessagingHub> _hub,
            AbpDzMessagingerviceConfig config,
        ...
        protected IHubContext<AbpDzMessagingHub> Hub { get; }
        public AbpDzMessagingerviceConfig Config { get; }
            ...
            string permission = null)
        {
            ...
            if (ContextAccessor != null && ContextAccessor.HttpContext != null && ContextAccessor.HttpContext.Request != null)
            {
                ContextAccessor.HttpContext.Request.Headers.TryGetValue("__signalr", out StringValues v);
        ...
        public async Task Notify(AbpDzMessagingInfo item, CrudOperation operation)
        {
            object data = null;
            switch (operation)
            {
        ...
        public async Task CreateMessaging(AbpDzMessagingInfo item, Boolean Persiste = true, int SendLevel = 0)
        {
            if (Persiste)
            {
